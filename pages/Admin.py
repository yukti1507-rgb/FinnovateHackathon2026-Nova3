...
from ui import apply_theme, navbar, page_header, require_admin, status_colors

ACTION_ICONS = {
    "LOGIN_FAILED": "✖", "ACCOUNT_LOCKED": "🔒", "LOGIN_BLOCKED": "⛔",
    "LOGIN_SUCCESS": "✔", "ACCOUNT_UNLOCKED": "🔓",
}
ACTION_GROUPS = {
    "LOGIN_FAILED": "bad", "ACCOUNT_LOCKED": "bad", "LOGIN_BLOCKED": "warn",
    "LOGIN_SUCCESS": "good", "ACCOUNT_UNLOCKED": "good",
}


def with_icon(action):
    """A symbol next to the action, so the table never relies on colour alone."""
    return f"{ACTION_ICONS.get(action, '•')} {action}"


def color_action(val):
    group = ACTION_GROUPS.get(str(val).split(" ", 1)[-1])
    if group is None:
        return ""
    background, text = status_colors()[group]
    return f"background-color:{background}; color:{text}"


def _result(outcome, verb):
    """lock_user/unlock_user may return True/False or (True/False, message) - accept both."""
    if isinstance(outcome, tuple):
        return outcome
    return bool(outcome), (f"Account {verb}." if outcome else f"Could not update this account.")
...
apply_theme()
navbar(active="Admin")
require_admin()          # non-admins never see this page, even by typing the address
page_header("Admin Dashboard", "Manage accounts and monitor security activity")
            ...
            if admin is None:
                ...
                if locked:
                    ...
                    if st.button(
                        ...
                        success, message = _result(unlock_user(conn, user_id, admin_id), "unlocked")
                ...
                # --------------------------------------------------
                    ...
                    if st.button(
                        ...
                        success, message = _result(lock_user(conn, user_id, admin_id), "locked")
...
with tab2:
    ...
    if logs:
        ...
        display_df["Action"] = display_df["Action"].map(with_icon)

        st.dataframe(
            display_df.style.map(color_action, subset=["Action"]),
