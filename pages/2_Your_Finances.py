...
main_income = st.number_input(
    "What's your main monthly income (take-home)?",
    min_value=0.0, step=100.0, key="income_input"
)

st.markdown("**Other sources of income**")
st.caption("Any extra money coming in — e.g. Freelance, Rental, Side business.")

for i, source in enumerate(st.session_state["other_income"]):
    with st.expander(f"{source['name']} — Rs {source['amount']}"):
        new_name = st.text_input("Where does it come from?", value=source["name"], key=f"income_name_{i}")
        new_amount = st.number_input(
            "Amount per month (Rs)", value=source["amount"], min_value=0.0, step=100.0, key=f"income_amount_{i}"
        )

        if new_amount <= 0:
            st.warning("⚠️ Amount must be greater than 0 — this won't be included until fixed.")
        else:
            st.session_state["other_income"][i] = {"name": new_name, "amount": new_amount}

        if st.button("🗑️ Remove", key=f"remove_income_{i}"):
            st.session_state["other_income"].pop(i)
            st.rerun()

with st.expander("➕ Add another source of income"):
    new_income_name = st.text_input(
        "Where does it come from?", placeholder="e.g. Freelance, Rental, Side business", key="new_income_name"
    )
    new_income_amount = st.number_input(
        "Amount per month (Rs)", min_value=0.0, step=100.0, key="new_income_amount"
    )

    if st.button("Add income source", type="primary"):
        if new_income_name.strip() == "":
            st.error("Please say where this income comes from.")
        elif new_income_amount <= 0:
            st.error("Amount must be greater than 0.")
        else:
            st.session_state["other_income"].append({"name": new_income_name, "amount": new_income_amount})
            st.success(f"Added '{new_income_name}'!")
            st.rerun()

other_income_total = sum(s["amount"] for s in st.session_state["other_income"])
income = main_income + other_income_total

col_i1, col_i2 = st.columns(2)
with col_i1:
    st.metric("Other income", f"{other_income_total:,.0f}")
with col_i2:
    st.metric("Total monthly income", f"{income:,.0f}")
...
rent, rent_is_need = expense_row("Rent", "fixed_rent", step=50.0)
food, food_is_need = expense_row(
    "Food & groceries", "fixed_food", step=50.0,
    help_text="Your usual monthly spend on groceries and meals at home."
)
...
single_fixed_total = rent + food + insurance + utilities + school_childcare + transport_fixed
...
st.metric("Total subscription amount (Rs)", f"{subscriptions_total:,.0f}")
...
fixed_categories = [
    {"name": "Rent", "amount": rent, "type": "Need" if rent_is_need else "Want"},
    {"name": "Food & groceries", "amount": food, "type": "Need" if food_is_need else "Want"},
