...
pic_col, emoji_col = st.columns(2)

with pic_col:
    st.markdown("**Upload a profile picture**")
    st.caption("If you upload a photo, it will be shown instead of your emoji avatar.")
    uploaded_file = st.file_uploader("Upload a profile picture", type=["png", "jpg", "jpeg"], label_visibility="collapsed")
    if uploaded_file:
...
with emoji_col:
    ...
    selected_avatar = st.selectbox("Choose an avatar", Avatar_options, index=i, label_visibility="collapsed")
